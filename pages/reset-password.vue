      ...
      <form class="mt-8 space-y-6" action="#" method="POST" @submit.prevent="handleSubmit">
        ...
        <div class="rounded-md shadow-sm -space-y-px">
          ...
          <div>
            ...
            focus:border-indigo-500 focus:z-10 sm:text-sm my-3" placeholder="New Password">
          </div>
          <div>
            <label for="confirm-password" class="sr-only">Confirm Password</label>
            <input id="confirm-password" name="confirm-password" type="password" v-model="confirmPassword"
            autocomplete="current-password" required class="appearance-none rounded-none relative block w-full px-3
            py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none
            focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm my-3" placeholder="Confirm Password">
          </div>
        </div>
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      let res = await this.$axios.post('/auth/password/reset', resetPassword).catch(err => this.printError(err))
    ...
    printError(error) {
      ...
      if (typeof errorObject === 'string' || errorObject instanceof String) {
        ...
        for (const [key, message] of Object.entries(error.response.data.message)) {
